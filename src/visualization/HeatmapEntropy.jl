module HeatmapEntropy

using QuantumOptics, SharedArrays, Distributed, Plots, NPZ, LinearAlgebra
using ..QuantumUtils: coherentspinstate, compute_rhoQ, compute_von_neumann_entropy
export vn_entropy_grid, density_plot

function vn_entropy_grid(s, U::Any, rdm_dim, itr, space; recompute_U_on_worker::Bool=false, floquet_args=nothing, max_bad_per_point::Int=5)
...
function density_plot(data, rdm_dim, space, s, params)
    θs, ϕs = space.θs, space.ϕs
    p, k, kp = params.p, params.k, params.kp
    # kr, kθ = (k+kp)/2, (k-kp)/2

    npzwrite("npy/entropy_j$(s)_partition$(rdm_dim)_p$(p)_k$(k)_kp$(kp).npy", data)
    heatmap(collect(ϕs), collect(θs), data, levels=500, color=:turbo,
        xlabel="ϕ", ylabel="θ", title="VN: j = $(s), k = $(k), kp = $(kp)")
    savefig("img/entropy_j$(s)_partition$(rdm_dim)_p$(p)_k$(k)_kp$(kp).png")
end
